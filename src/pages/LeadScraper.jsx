...
import {
  Search, Loader2, MapPin, Star, Phone, Globe, Mail, Plus,
  CheckSquare, Square, Zap, X, Sparkles
} from "lucide-react";
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto pb-24 md:pb-6">
      <div className="mb-4">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Find businesses by industry, location, keyword, and radius. Enrich and ingest into CRM.</p>
      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {lead.ingested_to_crm && <span className="text-[9px] text-emerald-500">✓ In CRM</span>}
                    </div>
                    ...
                    {lead.enrichment_data && Object.keys(lead.enrichment_data).length > 0 && (
                      <details className="mt-2">
                        ...
                        <pre className="mt-1 p-2 rounded bg-accent/30 text-[10px] text-foreground max-h-32 overflow-y-auto">{JSON.stringify(lead.enrichment_data, null, 2)}</pre>
                      </details>
                    )}
                  </div>
                  <div className="flex flex-col gap-1 shrink-0">
                    <button onClick={() => setShowEnrich(lead)} disabled={lead.enriched || enriching === lead.id}
                      className="px-2 py-1 rounded-lg border border-border text-xs hover:bg-accent disabled:opacity-50 flex items-center gap-1">
                      {enriching === lead.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3" />} Enrich
                    </button>
                  </div>
                </div>
              </div>
      ...
      {showEnrich && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowEnrich(null)}>
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-5" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2 mb-4">
              {ENRICHMENT_OPTIONS.map(opt => (
                <label key={opt.v} className="flex items-start gap-2 cursor-pointer">
                  <input type="checkbox" checked={enrichOptions.includes(opt.v)}
                    onChange={() => {
                      setEnrichOptions(prev => prev.includes(opt.v) ? prev.filter(v => v !== opt.v) : [...prev, opt.v]);
                    }}
                    className="mt-0.5" />
                  <div>
            ...
            <button onClick={() => enrichLead(showEnrich, enrichOptions)} disabled={enrichOptions.length === 0}
