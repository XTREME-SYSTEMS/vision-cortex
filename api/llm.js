...
function encodeWord(data, mimeWordEncoding, maxLength) {
  ...
  if (mimeWordEncoding === "Q") {
    ...
    encodedStr = typeof data === "string" ? data : encode2(data);
...
// base44/runtime/index.ts
var SUPABASE_URL = process.env.SUPABASE_URL || "";
var SUPABASE_ANON_KEY = process.env.SUPABASE_ANON_KEY || "";
var SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || "";
var AI_GATEWAY_URL = "https://ai-gateway.vercel.sh/v1";
async function supabaseFetch(path3, init, auth) {
  const res = await fetch(`${SUPABASE_URL}${path3}`, {
    ...init,
    headers: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Supabase ${res.status} on ${path3}: ${detail}`);
  }
  const text = await res.text();
  return text ? JSON.parse(text) : null;
}
function restEncode(value) {
  ...
  for (const [field, value] of Object.entries(params)) {
    ...
    if (value !== null && typeof value === "object" && !Array.isArray(value)) {
      for (const [op, opVal] of Object.entries(value)) {
        switch (op) {
          case "$in":
            parts.push(`${field}=in.(${opVal.map(restEncode).join(",")})`);
            break;
          case "$nin":
            parts.push(`${field}=not.in.(${opVal.map(restEncode).join(",")})`);
            break;
          case "$ne":
            parts.push(`${field}=neq.${restEncode(opVal)}`);
            break;
          case "$gt":
            parts.push(`${field}=gt.${restEncode(opVal)}`);
            break;
          case "$gte":
            parts.push(`${field}=gte.${restEncode(opVal)}`);
            break;
          case "$lt":
            parts.push(`${field}=lt.${restEncode(opVal)}`);
            break;
          case "$lte":
            parts.push(`${field}=lte.${restEncode(opVal)}`);
            break;
          case "$like":
...
function makeEntityClient(table, auth) {
  const tableQ = encodeURIComponent(`"${table}"`);
  return {
    async list(params = {}) {
      const q = buildFilterQuery(params);
      return supabaseFetch(`/rest/v1/${tableQ}?select=*&${q}`, { method: "GET" }, auth);
    },
    async filter(params = {}) {
...
async function sendEmail(payload) {
  ...
  const nodemailer2 = await Promise.resolve().then(() => (init_nodemailer(), nodemailer_exports));
...
async function uploadFile(payload) {
  ...
  const path3 = `${Date.now()}-${file_name}`;
  const buf = Buffer.from(content, "base64");
  const res = await fetch(`${SUPABASE_URL}/storage/v1/object/${bucket}/${encodeURIComponent(path3)}`, {
    method: "POST",
    headers: {
  ...
  return {
    file_url: `${SUPABASE_URL}/storage/v1/object/public/${bucket}/${encodeURIComponent(path3)}`,
    path: path3
  };
}
...
function makeConnectors() {
  return {
    async getConnection(name2) {
      const envName = `CONN_${String(name2).toUpperCase().replace(/[^A-Z0-9]/g, "_")}_ACCESS_TOKEN`;
      const token = process.env[envName];
      return token ? { access_token: token, provider: name2 } : null;
    }
  };
}
async function resolveUser(jwt) {
  if (!jwt) return null;
  try {
    ...
    let role = "user";
    let fullName = su.user_metadata?.full_name || su.user_metadata?.name || "";
    if (SERVICE_ROLE_KEY) {
      try {
        const rows = await supabaseFetch(
...
function createClientFromRequest(req) {
  let jwt = null;
  try {
    const authz = req.headers.get("authorization") || req.headers?.authorization;
  ...
  const userPromise = resolveUser(jwt);
  const client = {
    auth: {
      async me() {
        return userPromise;
